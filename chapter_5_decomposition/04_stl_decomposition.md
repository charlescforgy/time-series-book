...
2. Calculate the residuals. If $n_o>0$, calculate robustness weights to quantify each residual's degree of extremity for the LOESS algorithm (i.e. to what extent it should be considered an outlier) and return to step 1.1 until completing $n_o$ iterations, otherwise stop.
