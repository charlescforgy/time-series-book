...
where $u$ is the distance between the the focal point $x_0$ and the point $x_i$ calculated as $u=\frac{x_0-x_i}{\max_j{(|x_0-x_j|)}}$. The maximum $j$ is determined by an adjustable parameter giving the length of the kernel.  
...
For example, for $\max_j{(|x_0-x_j|)}=5$, the tricube function $W$ generates a kernel with $9$ non-zero values
...
with all points outside the window weighted as $0$ (note that for the first and last elements $u=\frac{\pm5}{5}$, resulting in zero for $W$).
