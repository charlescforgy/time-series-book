...
[^1]: Mathematically, ordinary least squares (OLS) regression is the maximum likelihood estimator for a linear model under the assumption of iid normal errors. However, unlike most MLE that relies on various optimization algorithms, OLS has a closed form exact solution. As a result, OLS usually lacks the numerical instability issues that frequently plague other MLEs.

For these reasons, AR models are often preferred over MA models when they give comparable results with a similar number of parameters[^2].

[^2]: We will explore methods for converting between ARMA, pure AR, and pure MA models in [section 4](04_arma.md).
