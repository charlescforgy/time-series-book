...
%For $\gamma_{x,y}(h)$ or $\rho_{x,y}(h)$, when $h>0$ we say $x_t$ *leads* $y_t$ as it is "in front" (or equivalently that $y_t$ *lags* $x_t$ as it's "in back"), whereas when $h<0$ we say $y_t$ leads (or $x_t$ lags).
%```
